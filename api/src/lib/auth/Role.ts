import { type Permission, type PermissionSchema, PermissionType } from './Permissions';
...
export function getDefaultRoleSchema(permSchema: PermissionSchema): RoleSchema {
  return {
    ...
    [RoleType.BEN]: {
      ...
      forGroups: [],
      permissions: [
        permSchema[PermissionType.VIEW_ADMIN_PANEL],
        permSchema[PermissionType.VIEW_DASHBOARD],
      ],
    },
    [RoleType.FRIEND]: {
...
export function getRolesFromGroup(schema: RoleSchema, ...groups: string[]): RoleType[] {
  ...
  for (const group of groups) {
    ...
    const rolesForGroup = Object.values(schema).filter(
